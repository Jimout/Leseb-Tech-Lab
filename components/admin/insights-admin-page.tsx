...
export function InsightsAdminPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: 'Delete failed',
        description: err instanceof Error ? err.message : 'Could not delete insight',
        variant: 'destructive',
      })
    } finally {
      setDeletingId(null)
                  ...
                  <TableRow key={it.id} className={cn('border-white/10')}>
                    ...
                    <TableCell className="whitespace-nowrap text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <ConfirmDeleteDialog
                          title="Delete insight?"
                          description={`Are you sure you want to delete "${it.title}"? This action cannot be undone.`}
